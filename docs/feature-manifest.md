...
| L1 `unmediated_io` detection — clock / rand / dns / network-unmediated (RFC-040) | 2 | `internal/star/builtins_determinism_test.go` (detection helpers + isMediatedAddress) + `internal/proxy/proxy_test.go` (IsListenPort, 6 tests) | 🟡 | Detection wires into the syscall callback for already-faulted services; end-to-end goldens (DNS leak, raw socket, clock read) deferred to a Linux-side follow-up — they need seccomp + a real binary |
