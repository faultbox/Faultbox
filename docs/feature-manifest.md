...
| A declared fault either fires or the run fails | 1 | `internal/star/fault_coverage_test.go` (coverage check, open→openat mirror, nil-session case) + `internal/star/spec_statements.go` + both spellings and the variable-built case verified end-to-end in Lima | 🟢 | v0.18.0. Which syscalls are intercepted is chosen by a source scan before the run, and that scan is a heuristic — multi-line and spaced `fault()` now resolve, a helper wrapping one resolves (the scan is file-wide), but rules passed in from a variable cannot. What makes the heuristic safe is the check at install time: a fault targeting a syscall the filter does not cover is **refused** (`FAULT_NOT_FILTERABLE`) rather than installed inert. Measured: the variable-built form went from "1 passed" with a warning to a hard failure naming the syscalls and the fix |
