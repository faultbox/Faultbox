...
type FaultScenarioDef struct {
	...
	// RequireFaultsFire, when true, demotes the row's outcome from
	// "passed" to "fault_bypassed" if the event log contains any
	// fault_zero_traffic events — i.e. the fault was installed but the
	// scenario never matched a syscall. Opt-in gate for resilience
	// claims; defaults to false so existing matrices keep their
	// current semantics. Wired by fault_matrix(require_faults_fire=True).
